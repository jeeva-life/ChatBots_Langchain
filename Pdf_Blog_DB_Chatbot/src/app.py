...
async def main():
    async def storeDocEmbeds(file, filename): # for PDF
        ...
        try:
            ...
            vectors.save_local(filename + '.faiss')
    ...
    async def getDocEmbbs(file, filename): # for PDF
        ...
        try:
            embeddings = OpenAIEmbeddings(openai_api_key=api_key)  # Ensure the same embeddings instance is used
            vectors = FAISS.load_local(index_file, embeddings)
        except Exception as e:
    ...
    if option == "PDF":
        ...
        if uploaded_file is not None:
            with st.spinner("processing..."):
                ...
                #Check if vectors are loaded carefully
                if vectors is None:
                    st.error("Failed to load document embeddings. Please try again.")
                else:
                    try:
